...
class MimeTypeMap private constructor() {
    ...
    fun getMimeTypeFromUrl(url: String): String {
        ...
        if (extension.isNotEmpty()) {
            type = getMimeTypeFromExtension(extension) ?: type
        }
        return type
    }

    companion object {
        private var sMimeTypeMap: MimeTypeMap? = null
        ...
        fun getFileExtensionFromUrl(url: String?): String {
            if (url != null && url.isNotEmpty()) {
                var urlString = url
                val query = urlString.lastIndexOf('?')
                if (query > 0) {
                    urlString = urlString.substring(0, query)
                }
                val filenamePos = urlString.lastIndexOf('/')
                val filename = if (0 <= filenamePos) urlString.substring(filenamePos + 1) else urlString
